...
Version 2.0.0 uses the Prowler AWS CLI and mutelist contract. Replace `prowler_allowlist_file` with
`prowler_mutelist_file`, and convert the YAML root key from `Allowlist:` to `Mutelist:`. The module
keeps the existing S3 object key to avoid replacing report storage during the migration.
...
| <a name="input_codebuild_image"></a> [codebuild\_image](#input\_codebuild\_image) | (Required) Docker image to use for this build project. | `string` | `"aws/codebuild/amazonlinux-x86_64-standard:6.0"` | no |
...
| <a name="input_prowler_cli_options"></a> [prowler\_cli\_options](#input\_prowler\_cli\_options) | Additional options passed to the Prowler AWS CLI. | `string` | `"-S --compliance aws_foundational_security_best_practices_aws aws_well_architected_framework_security_pillar_aws cis_3.0_aws aws_audit_manager_control_tower_guardrails_aws aws_well_architected_framework_reliability_pillar_aws soc2_aws mitre_attack_aws --output-formats html json-ocsf --send-sh-only-fails --no-banner --ignore-exit-code-3"` | no |
...
| <a name="input_prowler_mutelist_file"></a> [prowler\_mutelist\_file](#input\_prowler\_mutelist\_file) | Prowler mutelist file. See https://docs.prowler.com/user-guide/cli/tutorials/mutelist/. | `string` | `null` | no |
...
| <a name="input_prowler_version"></a> [prowler\_version](#input\_prowler\_version) | Prowler CLI version. | `string` | `"5.42.0"` | no |
